...
# --- Fetch schedule and tie games to their date headers ---
def fetch_schedule():
    """Scrape FantasyData NHL schedule and tie each game to its date header."""
    try:
    ...
    for tr in table.find_all("tr"):
        ...
        date_text = None
        header_candidate = tr.find("th") or tr.find("td", {"colspan": True})
        if header_candidate:
            date_text = header_candidate.get_text(strip=True)
        else:
            plain_text = tr.get_text(strip=True)
            if re.match(r"^[A-Za-z]+,?\s+[A-Za-z]+\s+\d{1,2}$", plain_text):
                date_text = plain_text

        if date_text:
            for fmt in ("%A, %b %d", "%A, %B %d", "%b %d", "%B %d"):
                try:
                    parsed_dt = dt.datetime.strptime(date_text, fmt)
        ...
        text_line = " ".join(cols)
        match = re.search(r'(.+?)\s*@\s*(.+?)\s+(\d{1,2}:\d{2}\s*(?:AM|PM)?)', text_line)
        if match:
            away, home, game_time = match.groups()
        else:
            away, home = cols[0], cols[1]
            game_time = cols[-1] if re.search(r'\d{1,2}:\d{2}', cols[-1]) else None
        ...
        if current_date:
            rows_data.append({
                ...
                "away": away.strip(),
                "home": home.strip(),
                "game_time": game_time.strip() if game_time else "",
...
if not schedule_df.empty:
    ...
    selected_date = st.date_input("Select date:", dt.datetime.now().date(),
...
if schedule_df.empty:
    ...
    st.header(f"Schedule for {selected_date.strftime('%A, %b %d')}")
