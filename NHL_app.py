...
# --- Config ---
DAILYFANTASYFUEL_URL = "https://www.dailyfantasyfuel.com/nhl/matchup-analysis/"
REQUESTS_TIMEOUT = 10
...
def fetch_schedule():
    """Scrape Daily Fantasy Fuel NHL matchup schedule"""
    try:
        r = requests.get(DAILYFANTASYFUEL_URL, headers=HEADERS, timeout=REQUESTS_TIMEOUT)
        r.raise_for_status()
        time.sleep(REQUEST_DELAY)
    except Exception as e:
        st.error(f"Failed to fetch Daily Fantasy Fuel schedule: {e}")
        return pd.DataFrame()

    soup = BeautifulSoup(r.text, "lxml")
    schedule_rows = soup.select("table tbody tr")
    if not schedule_rows:
        st.warning("Could not find schedule table on Daily Fantasy Fuel page.")
        return pd.DataFrame()
    ...
    for tr in schedule_rows:
        ...
        if len(tds) < 3:
            continue
        # Assume table format: Time | Away | Home
        ...
        rows_data.append({
            ...
            "home": home,
            "game_time": time_text,
            "home_abbr": TEAM_ABBR_MAP.get(home, home),
            "away_abbr": TEAM_ABBR_MAP.get(away, away)
        })
